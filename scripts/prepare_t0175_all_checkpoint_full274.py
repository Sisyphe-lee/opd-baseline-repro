#!/usr/bin/env python3
"""Generate frozen seed-42 full274 configs for tau=0.175 checkpoints."""
...
RUN_ROOT = ROOT / "runs" / "experiments" / "t0175_all_checkpoint_full274_seed42"
EXPORT_ROOT = ROOT / "runs" / "exports" / "t0175_all_checkpoint_curve"
MANIFEST_ROOT = ROOT / "data" / "eval_manifests"
MISSING_STEPS = (20, 40, 60, 80, 100, 120, 140, 160, 180, 200, 220, 240)


def render(step: int) -> str:
    method = "entropy_adaptive_v1_t0175"
    label = f"allckpt_{method}_step{step}"
    run_root = RUN_ROOT / f"step_{step}_seed42"
    model_path = EXPORT_ROOT / f"step_{step}"
...
group: "t0175-all-checkpoint-full274-seed42"
name: "{label}-full274-seed42-h30-5gpu"
...
def main() -> None:
    ...
    for step in MISSING_STEPS:
        path = CONFIG_DIR / f"allckpt_entropy_adaptive_v1_t0175_step{step}_full274_seed42.yaml"
        path.write_text(render(step), encoding="utf-8")
    print(f"Generated {len(MISSING_STEPS)} frozen full274 configs under {CONFIG_DIR}")
