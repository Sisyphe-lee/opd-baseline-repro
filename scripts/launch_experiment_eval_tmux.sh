...
tmux respawn-pane -k -t "${SESSION}" \
  "bash -lc \"set -o pipefail; cd '${ROOT}' && bash scripts/_run_experiment_eval.sh '${CONFIG}' '${GPU_IDS}' '${RUN_TAG}' '${RAY_PORT}' '${RUN_ROOT}' 2>&1 | tee '${RUN_ROOT_ABS}/logs/tmux.log'; status=\\\${PIPESTATUS[0]}; printf '%s\\\\n' \\\"\\\${status}\\\" > '${RUN_ROOT_ABS}/logs/exit_status'; exit \\\"\\\${status}\\\"\""
