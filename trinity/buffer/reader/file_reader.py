...
def _load_task_dataset(config: StorageConfig) -> Dataset:
    """Load task dataset from path. Supports both HF dataset and parquet/jsonl file path."""
    path = config.path
    split = config.split
    if path and str(path).endswith(".parquet"):
        return load_dataset(
...
class TaskFileReader(BaseFileReader):
    ...
    def __init__(self, config: StorageConfig):
        ...
        self.read_batch_size = config.batch_size
        self.dataset = _HFBatchReader(
            _load_task_dataset(self.config),
